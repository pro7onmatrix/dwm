...
enum { SchemeNorm, SchemeInv, SchemeSel, SchemeOcc, SchemeWarn, SchemeUrgent }; /* color schemes */
...
drawbar(Monitor *m)
{
  ...
  for (i = 0; i < LENGTH(tags); i++) {
    ...
    if (m == selmon) {
      ...
      drw_setscheme(drw, scheme[(m->tagset[m->seltags] & 1 << i) ? SchemeInv : (occ & 1 << i) ? SchemeOcc : SchemeNorm]);
    }
...
monocle(Monitor *m)
{
  ...
  if (n > 0) /* override layout symbol */
    snprintf(m->ltsymbol, sizeof m->ltsymbol, "%s", n < 10 ? monoclelabels[n - 1] : monoclelabels[9]);
